...
const createProperty = async (req, res) => {
    try {
        ...
        const { title, description, price, location, bedrooms, bathrooms, is_premium, amenities, max_guests, beds } = req.body;
        const imageUrls = req.body.imageUrls; 

        // Validate required fields
        if (!title || !description || !price || !location || !bedrooms || !imageUrls || imageUrls.length === 0 || max_guests === undefined || beds === undefined) {
            return res.status(400).json({ success: false, message: "All fields including images, max_guests, and beds are required" });
        }
        ...
        const insertPropertyQuery = `
            INSERT INTO feature_property (title, description, price, location, bedrooms, bathrooms, is_premium, max_guests, beds)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        const propertyResult = await new Promise((resolve, reject) => {
            db.query(insertPropertyQuery, [title, description, price, location, bedrooms, bathrooms, is_premium, max_guests, beds], (err, result) => {
...
const getAllProperties = async (req, res) => {
    try {
        // Step 1: Fetch properties (including max_guests and beds)
        const propertyQuery = `SELECT id, title, description, price, location, bedrooms, bathrooms, beds, is_premium, max_guests, created_at FROM feature_property`;
        ...
        const formattedProperties = properties.map(property => {
            const { id, title, description, price, location, bedrooms, bathrooms, beds, is_premium, max_guests, created_at } = property;

            // Get the related images and amenities
            ...
            return {
                ...
                bedrooms,
                bathrooms,
                beds,  // Added beds here
                is_premium,
                max_guests, // Include max_guests in the response
                created_at,
...
const getPropertyById = async (req, res) => {
    try {
        ...
        // Initialize property details, including beds
        const formattedProperty = {
            ...
            beds: property[0].beds,  // Added beds to the response
            is_premium: property[0].is_premium,
            created_at: property[0].created_at,
            max_guests: property[0].max_guests,
            images: [],
            amenities: [],
        };

        // Process images and avoid duplicates
        property.forEach(prop => {
            const { image_id, image_url, amenity } = prop;

            // Add images if not already added
            if (image_id && image_url) {
                const imageExists = formattedProperty.images.some(image => image.id === image_id);
                if (!imageExists) {
                    formattedProperty.images.push({ id: image_id, url: image_url });
                }
            }

            // Add amenities if not already added
            if (amenity) {
                const amenityExists = formattedProperty.amenities.includes(amenity);
                if (!amenityExists) {
                    formattedProperty.amenities.push(amenity);
                }
            }
        });
...
const getFilteredProperties = async (req, res) => {
    try {
        ...
        // Max guests filter
        if (max_guests) {
            query += ` AND p.max_guests <= ?`;  // Ensure max_guests is less than or equal to the value passed
        ...
        const formattedProperties = properties.reduce((acc, prop) => {
            const { id, title, description, price, location, bedrooms, bathrooms, is_premium, max_guests, created_at, image_id, image_url, amenity } = prop;
            ...
            if (!existingProperty) {
                existingProperty = {
                    ...
                    bathrooms,
                    is_premium,
                    max_guests, // Include max_guests in response
                    created_at,
                    images: [],
                    amenities: [],
                };
...
const updateProperty = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, price, location, bedrooms, bathrooms, beds, is_premium, max_guests, amenities } = req.body;
        const imageUrls = req.body.imageUrls; // Get image URLs from middleware

        // Input validation
        if (!title || !description || !price || !location || !bedrooms || !beds) {
        ...
        try {
            const updateQuery = `
                ...
                SET 
                    ...
                    bedrooms = ?, 
                    bathrooms = ?, 
                    beds = ?,  -- Added beds to the update query
                    is_premium = ?, 
                    max_guests = ?   -- Added max_guests to the query
                WHERE id = ?
            `;
            ...
            await new Promise((resolve, reject) => {
                db.query(
                    updateQuery, 
                    [title, description, price, location, bedrooms, bathrooms, beds, isPremiumValue, max_guests, id], 
                    (err) => {
                        if (err) return reject(err);
                        resolve();
                    }
